...
def get_assessment_service(llm=Depends(get_ollama_instance)):
    return AssessmentService(llm_provider=llm)

async def _get_user_profile(current_user: dict) -> dict:
    from app.core.database import get_supabase
    db = get_supabase()
    user_id = current_user["id"]
    profile = db.table("user_profiles").select("*").eq("user_id", user_id).limit(1).execute()
    prefs = db.table("user_preferences").select("*").eq("user_id", user_id).limit(1).execute()
    return {**current_user, **(profile.data[0] if profile.data else {}), **(prefs.data[0] if prefs.data else {}), "user_id": user_id}
...
async def get_status(
    ...
    from app.core.database import get_supabase
    eligibility = await service.check_retake_eligibility(current_user["id"])
    user = get_supabase().table("users").select("quick_assessment_done").eq("id", current_user["id"]).limit(1).execute()
    has_completed = user.data[0]["quick_assessment_done"] if user.data else False
    return ok(data={**eligibility, "can_retake": eligibility["eligible"], "has_completed": has_completed})
